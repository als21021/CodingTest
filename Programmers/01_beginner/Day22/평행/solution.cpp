...
/*
풀이
평행을 찾기 위해서는 기울기를 찾아야 함
점이 네 개면 선분이 총 6개가 나오는데 6개의 기울기를 하나하나 다 찾을까?
그리고 같은 기울기가 있으면 1을 리턴
근데 중복되는거 어떻게 찾더라
기울기는 (x1 - x2) / (y1 - y2)
*/
...
int solution(vector<vector<int>> dots) {
    ...
    if(((double)(dots[0][1] - dots[1][1]) / (dots[0][0] - dots[1][0])) == ((double)(dots[2][1] - dots[3][1]) / (dots[2][0] - dots[3][0]))) {
        return 1;
    } else if(((double)(dots[0][1] - dots[3][1]) / (dots[0][0] - dots[3][0])) == ((double)(dots[2][1] - dots[1][1]) / (dots[2][0] - dots[1][0]))) {
        return 1;
